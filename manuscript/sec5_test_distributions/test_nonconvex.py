...
a = np.array([(-1)**i for i in range(dim)])
target = DenseArrayDistribution.get_nonconvex(grid, a)
...
solver.params.max_rank = 2  # Max TT rank for all the represented variables
solver.params.cross_validation_rtol = 0  # disable validation
solver.params.cross_rel_diff = 1e-2
solver.params.fp_stopping_rtol = 1e-7
solver.params.cross_nfev_with_posterior = int(2e4)
...
fname = "./mcmc_results/6d_nonconvex.h5"
if os.path.exists(fname):
    ...
    sampler = emcee.EnsembleSampler(
        ...
        dim,
        target.log_density,
        moves=[(emcee.moves.GaussianMove(cov=0.04), 1.0)],
        backend=backend,
    )
    state = sampler.run_mcmc(init, 30_000, progress=True)
    mcmc_result = sampler
    print(
        np.min(mcmc_result.acceptance_fraction), np.max(mcmc_result.acceptance_fraction)
    )
...
samples_ref = samples_ref[-2 * n_draws :]
...
opath = "./outputs/6d_nonconvex.npz"
np.savez(
    opath,
    dim=dim,
    n_cache=solver.n_cache,
